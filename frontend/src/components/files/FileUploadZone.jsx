...
const CHUNK_SIZE = 8 * 1024 * 1024;
const MAX_CONCURRENT = 3; // Upload 3 files simultaneously
...
const FileUploadZone = ({ onUploadComplete }) => {
  ...
  const checkAllComplete = useCallback(() => {
    ...
    if (completedFilesRef.current >= totalFilesRef.current) {
      // All files processed — notify parent
      if (!hasErrorRef.current) {
        toast({ title: 'All uploads complete', description: `${totalFilesRef.current} file(s) uploaded successfully` });
      }
  ...
  // Upload a single file (called concurrently)
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    // Create queue entries for ALL files immediately
    ...
    const filesToProcess = newEntries.map(entry => ({
      file: entry.file,
      fileId: entry.id,
    }));
    ...
    const startNext = () => {
      while (activeUploadsRef.current < MAX_CONCURRENT && queue.length > 0) {
        ...
        uploadSingleFile(file, fileId).then(() => {
          startNext(); // When one finishes, start the next
        });
      }
    };
  ...
  const retryUpload = (fileId) => {
    ...
    if (fileItem?.file) {
      removeFromQueue(fileId);
      // Re-upload the stored file reference
      const newId = `${Date.now()}-${Math.random().toString(36).slice(2)}-${fileItem.name}`;
