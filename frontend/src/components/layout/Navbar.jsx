    ...
    <header className="w-full px-3 pt-3 sm:px-6 lg:px-8 font-mono">
      <div className="mx-auto w-full max-w-[1720px] border border-border bg-background">
        <div className="relative">
          ...
          {/* Top Bar */}
          <div className="flex min-h-[56px] items-center justify-between px-5 py-3 sm:px-8 lg:px-10">
            {/* Logo */}
              ...
              <nav className="flex flex-col px-5 py-5">
                {navLinks.map((link) => (
                  <a
                    ...
                    className="border-b border-border py-3 text-sm text-foreground/70 transition hover:text-foreground"
                  >
                    {link.label}
                  </a>
                ))}
